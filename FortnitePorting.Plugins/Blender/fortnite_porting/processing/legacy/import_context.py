...
class LegacyMaterialImportContext:
    ...
    def import_material(self, material_slot, material_data, meta, as_material_data=False):
        ...
        if not as_material_data:
            ...
            legacy_export_type = EExportType[self.type.name]
            material_slot.link = 'OBJECT' if legacy_export_type in [EExportType.WORLD, EExportType.PREFAB] else 'DATA'
        ...
        textures = material_data.get("Textures") or []
        scalars = material_data.get("Scalars") or []
        vectors = material_data.get("Vectors") or []
        switches = material_data.get("Switches") or []
        component_masks = material_data.get("ComponentMasks") or []

        texture_data = meta.get("TextureData")
        if texture_data is not None:
            # In V4, TextureData may replace the material in slot 0.
            if material_data.get("Slot") == 0:
                for data in texture_data:
                    if override_material := data.get("OverrideMaterial"):
                        material_data = override_material
                        material_name = material_data.get("Name")
                        material_hash = material_data.get("Hash")
                        textures = material_data.get("Textures") or []
                        scalars = material_data.get("Scalars") or []
            ...
            for data in texture_data:
                additional_hash += data.get("Hash") or 0
                ...
                for name, texture in (
                    (f"Diffuse{texture_suffix}", data.get("Diffuse")),
                    (f"Normals{texture_suffix}", data.get("Normal")),
                    (f"SpecularMasks{spec_suffix}", data.get("Specular")),
                ):
                    if texture:
                        texture_parameter = (
                            texture if isinstance(texture, dict) and texture.get("Name")
                            else {"Name": name, "Texture": texture}
                        )
                        replace_or_add_parameter(textures, texture_parameter)
        ...
        legacy_export_type = EExportType[self.type.name]
        shader_node.node_tree = bpy.data.node_groups.get("FPv3 Material Lite" if legacy_export_type in lite_shader_types else "FPv3 Material")
        ...
        def texture_param(data, target_mappings, target_node=shader_node, add_unused_params=False):
            try:
                ...
                path = texture.get("Path") if isinstance(texture, dict) else data.get("Value") or texture
                srgb = texture.get("sRGB") if isinstance(texture, dict) else data.get("sRGB")
                if not path:
                    return
                ...
                if image is None:
                    nodes.remove(node)
                    Log.warn(f"Texture file was not found: {path}")
                    return
                node.image = image
